...
describe("parseSchema", () => {
  ...
  FOREIGN KEY (product_id) REFERENCES products(id)
			...
			expect(result.tables[0].primaryKey.sort()).toEqual([
				"order_id",
				"product_id",
			]);
		});
...
CREATE TABLE reviews (id SERIAL PRIMARY KEY, book_id INTEGER REFERENCES books(id), rating INTEGER);`;
		...
		it("generates warnings for unrecognized types", () => {
			...
			expect(
				result.issues.some((i) => i.message.includes("WHATEVER_TYPE")),
			).toBe(true);
		});

		it("parses self-referencing tables (circular reference on same table)", () => {
  ...
  manager_id INTEGER REFERENCES employees(id)
			...
			expect(result.tables[0].foreignKeys).toHaveLength(1);
			const fk = result.tables[0].foreignKeys[0];
			expect(fk.column).toBe("manager_id");
			expect(fk.referencedTable).toBe("employees");
			expect(fk.referencedColumn).toBe("id");
		});
	});
...
ORDER BY event;`;
			const result = parseSchema(ddl);
			expect(result.tables).toHaveLength(1);
			const countryCol = result.tables[0].columns.find(
				(c) => c.name === "country",
			);
...
describe("lintQuick", () => {
	...
	it("returns no issues for valid DDL", () => {
		...
		const typeIssues = issues.filter((i) =>
			i.message.includes("Unrecognized type"),
		);
